...
def completeCheckRankingProperties(_ranking,_protected_group):    
    ...
    if isinstance(_ranking, (list, tuple, np.ndarray)) and not isinstance( _ranking, basestring ):
        raise TypeError("Input ranking must be a list-wise structure defined by '[]' symbol")
    if isinstance(_protected_group, (list, tuple, np.ndarray)) and not isinstance( _protected_group, basestring ):
