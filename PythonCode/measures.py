...
def calculateNDFairness(_ranking,_protected_group,_cut_point,_gf_measure,_normalizer):
    """
        ...
        :param _cut_point: Batch size for the calculation of group fairness, e.g., 10
    ...
    """
    # check that batch_size does not exceed the size of the ranking
    # check that there is never division by 0
    # check that _ranking is a valid permutation, and that _protected_group.size = 
    # _protected_group.intersect(_ranking).size
...
def calculateFairness(_ranking,_protected_group,_user_N,_pro_N,_gf_measure):
    ...
    # make 3 functions, one for each measure, call them from here
    # don't hard-code names here, hard-code constants at the top of the file, 
    # or as a separate file, and refer to them here
...
def getNormalizer(_user_N,_pro_N,_gf_measure):
    ...
    current_normalizer_key=str(_user_N)+","+str(_pro_N)+","+str(_gf_measure)
...
def readNormalizerDictionary():
    ...
    try:
        ...
        print("Cannot find the normalier file")
...
def calculateNormalizer(_user_N,_pro_N,_gf_measure):
    ...
    for fpi in f_probs:
        ...
        for iteri in range(NORM_ITERATION):
            ...
            gf=calculateNDFairness(unfair_ranking,protected_group,_cut_point,_gf_measure,1)
...
def calculateScoreDifference(_scores1,_scores2):
    """
        ...
        Only applied for two score lists with same size. - EITHER CHECK 
        LIST SIZES OR GO TO MIN(x1.size, x2.size)
    ...
    """
    # error handling 
    if isinstance(_scores1, (list, tuple, np.ndarray)) and not isinstance( _scores1, basestring ):
        raise TypeError("First score list must be a list-wise structure defined by '[]' symbol")
    if isinstance(_scores2, (list, tuple, np.ndarray)) and not isinstance( _scores2, basestring ):
...
def descendingOrderCheck(_ordered_list):
    ...
    """   
    for i in xrange(len(_ordered_list) - 1):
        if _ordered_list[i]<_ordered_list[i+1]:
            return False
    return True
...
def completePermutaionCheck(_perm1,_perm2):
    ...
    if isinstance(_perm1, (list, tuple, np.ndarray)) and not isinstance( _perm1, basestring ):
        raise TypeError("First permutation must be a list-wise structure defined by '[]' symbol")
    if isinstance(_perm2, (list, tuple, np.ndarray)) and not isinstance( _perm2, basestring ):
        raise TypeError("Second permutation must be a list-wise structure defined by '[]' symbol")
    if not descendingOrderCheck(_perm1):
        raise ValueError("First permutation is not ordered by descending order")
    if not descendingOrderCheck(_perm2):
        raise ValueError("Second permutation is not ordered by descending order")
