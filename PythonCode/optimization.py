...
def calculateEvaluateRez(_rez,_data,_inputscores,_k,_accmeasure):
    ...
    if isinstance(_rez, (list, tuple, np.ndarray)) and not isinstance( _rez, basestring ):
        raise TypeError("Input parameter list must be a list-wise structure defined by '[]' symbol")
    if isinstance(_inputscores, (list, tuple, np.ndarray)) and not isinstance( _inputscores, basestring ):
...
def calculateEstimateY(_M_nk_x, _inputscores, _clusters, _N, _k,_accmeasure):
    ...
    ranking_loss = 0.0       
    # generate permutations of two score lists
    ...
    if _accmeasure==SCORE_DIVERGENCE:
        L_y=measures.calculateScoreDifference(score_hat,_inputscores) 
        ranking_loss = L_y
    ...
    # for spearman and pearson relation, use the negative value to minimize during optimization
        ...
        L_y=measures.calculateSpearmanR(score_hat,_inputscores)[0] 
        ranking_loss = -L_y 

    elif _accmeasure==PEARSON_COR:
        L_y=measures.calculatePearsonC(score_hat,_inputscores)[0]
        ranking_loss=-L_y
